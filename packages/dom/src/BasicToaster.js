...
const BasicToaster = () => {
  ...
  const expandableElementId = useUniqueId('webchat__toaster__list');
  const headerElementId = useUniqueId('webchat__toaster__header');

  const expandableElementIdIfNeeded = !expandable || expanded ? expandableElementId : undefined;
  const headerElementIdIfNeeded = expandable ? headerElementId : undefined;
  ...
  return (
    <div
      aria-labelledby={headerElementIdIfNeeded}
      aria-live="polite"
      aria-relevant="all"
    ...
    >
      {expandable && (
        <button
          aria-controls={expandableElementIdIfNeeded}
          aria-expanded={expanded}
          className="webchat__toaster__header"
          id={headerElementIdIfNeeded}
      ...
      {(!expandable || expanded) && (
        <ul
          ...
          id={expandableElementIdIfNeeded}
          role="region"
        >
